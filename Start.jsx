#target photoshop
        ...
        function str(t, e) {
            ...
            switch (i && "object" == typeof i && "function" == typeof i.toJSON && (i = i.toJSON(t)), "function" == typeof rep && (i = rep.call(e, t, i)), typeof i) {
                ...
                case"object":
                    ...
                    if (rep && "object" == typeof rep) for (u = rep.length, r = 0; r < u; r += 1) "string" == typeof rep[r] && (n = rep[r], o = str(n, i), o && f.push(quote(n) + (gap ? ": " : ":") + o)); else for (n in i) Object.prototype.hasOwnProperty.call(i, n) && (o = str(n, i), o && f.push(quote(n) + (gap ? ": " : ":") + o));
                    return o = 0 === f.length ? "{}" : gap ? "{\n" + gap + f.join(",\n" + gap) + "\n" + a + "}" : "{" + f.join(",") + "}", gap = a, o
            }
        }
    ...
    var jsonAddress ="C://viditor//mockup//json.json";
	
	var thisFile = File(jsonAddress);
	
	thisFile.open("r");

	thisFile.encoding = "UTF-8";

	var fileJson = thisFile.read();

	arrayJson = JSON.parse(fileJson);
    
    var exportSrc=arrayJson.exportSrc;
   
    var arrayObject=arrayJson["objects"];
    
    var templateSrc=arrayJson["templateSrc"];
  
    var template=File(templateSrc);
    
    var document = app.open (template);
     ...
     var imageSrcReplace;
                   ...
                   function replaceContents(newFile, layerReplace) {
                       
                    document.activeLayer = layerReplace;
                    // =======================================================
        ...
        if(document.layers.length>0){
                        ...
                        for(var i=0;i<arrayObject.length;i++){
                                        ...
                                        var layerType =arrayObject[i]["type"] ;
                                         ...
                                         switch (layerType) {
                                                 ...
                                                 case "imageHolder":
                                                        ...
                                                        for ( j=0; j <document.layers.length; j++) {
                                                            ...
                                                            layerSelect = document.layers[j];
                                                            
                                                            if(layerSelect.name== arrayObject[j]["name"] && layerSelect.kind == "LayerKind.SMARTOBJECT" ){
                                                                
                                                                      layerReplace=layerSelect;
                                                                      
                                                                      imageSrcReplace=arrayObject[j]["imageSrc"];
                                                                      
                                                                     replaceContents(imageSrcReplace, layerReplace);
                                ...
                                var exportSrc=arrayJson["exportSrc"];
                                document.saveAs((new File(exportSrc)),jpgSaveOptions, true,Extension.LOWERCASE);
                                
                                var theNewName = "replaceFootager";
